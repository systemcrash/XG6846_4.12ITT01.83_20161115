...
#if	defined(XAVI_CFE_MULTICASTBOOT) || defined(CONFIG_XAVI_CFE_MULTICASTBOOT)
#define DEFAULT_XAVI_CFE_DEFAULTSET	0
#define XAVI_CFE_DEFAULTSET_TRUE	1
#define XAVI_CFE_DEFAULTSET_FALSE	0
#endif
...
#if defined(CONFIG_XAVI_FACTORY_PRODUCE_DEF)
#define BUFLEN_32 32                        /* same as userspace/public/include/cms.h */
#define BLOCK_NO  4
    char chUnused[298 - (BUFLEN_32 * BLOCK_NO) - 1];
    unsigned char ucNotResetDefaultConfig;  /* flag to indicate has reset default config or not */
    unsigned char modelName[BUFLEN_32];     /* null-terminated, so maximum is 31 digits */
    unsigned char desKey[BUFLEN_32];        /* null-terminated, so maximum is 31 digits */
    unsigned char wepKey[BUFLEN_32];        /* null-terminated, so maximum is 31 digits. in fact, 5/10 or 13/26 digits */
    unsigned char serialNumber[BUFLEN_32];  /* null-terminated, so maximum is 31 digits */
#else
    char chUnused[298];
#endif
