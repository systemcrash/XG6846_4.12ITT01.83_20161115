...
#ifdef XAVI_LAN_WAN_ACCESS
{
   ...
   char *pIpAddr;
#ifdef DMP_X_BROADCOM_COM_IPV6_1 /* aka SUPPORT_IPV6 */
   inet_ntop(AF_INET6, &from.sin6_addr, ipAddrBuf, sizeof(ipAddrBuf));
...
#else
   strcpy(ipAddrBuf, inet_ntoa(from.sin_addr));
   pIpAddr = &ipAddrBuf[0]; 
#endif

   int accessMode = cmsNet_isAddressOnLanSide(pIpAddr) ?
                    NETWORK_ACCESS_LAN_SIDE : NETWORK_ACCESS_WAN_SIDE;

   (cmsNet_isInAccessControlList(pIpAddr) != TRUE) ?
       (accessMode = NETWORK_ACCESS_DISABLED):
       ((cmsNet_isServiceEnabled("tftp", accessMode) != FALSE) ? :
           (accessMode = NETWORK_ACCESS_DISABLED));
