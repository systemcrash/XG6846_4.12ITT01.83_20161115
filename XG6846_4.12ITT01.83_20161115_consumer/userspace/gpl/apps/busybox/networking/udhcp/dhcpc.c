...
static char subnetmask[16]  = "";
static char routerip[16]    = "";
#endif
#ifdef XAVI_UDHCPC_OPTION
static char option_66[32]   = "";
static char option_67[32]   = "";
static char option_43[256]  = "";
static char option_121[256] = "";
static char option_125[256] = "";
static char option_128[256] = "";
static char option_132[32]  = "";
#ifdef XAVI_UDHCPC_OPT224
static char option_224[256] = "";
#endif
#ifdef XAVI_UDHCPC_OPT250_APAS
static char option_250[256] = "";
#endif
#endif
...
#ifdef XAVI_UDHCPC_OPTION
				if (!strcmp(opt_name, "staticroutes")) {
					uint8_t subnet_mask[4];
					uint32_t i = 0;

					/* print ip */
					opt121p += sprint_nip(opt121p, "", (void*) &nip);
					/* print mask */
					for (i = 0; i < 4; i++)
						subnet_mask[i] = (0xFFFFFFFF << (32 - mask)) >> (8 * (3 - i)) & 0xff;
					opt121p += sprint_nip(opt121p, " ", subnet_mask);
					/* print router */
					opt121p += sprint_nip(opt121p, " ", (option - 4));
					strcat(opt121p++, "\n");
				}
#endif
			}
...
static void config_save(void)
{
	...
	strcpy(interface, WAN_IF_NAME);
#else
	strcpy(interface, client_config.interface);
#endif

	sprintf(tmp_data, "mkdir -p %s/%s", _PATH_WAN_DIR, interface);
	system(tmp_data);

	sprintf(tmp_data, "%s/%s", _PATH_WAN_DIR, _PATH_WAN_IF);
	option_data_file_save(tmp_data, interface);

	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_IP);
	option_data_file_save(tmp_data, ipaddress);
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_MASK);
	option_data_file_save(tmp_data, subnetmask);
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_ROUTER);
	option_data_file_save(tmp_data, routerip);
#if defined(XAVI_UDHCPC_OPTION)
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_43);
	option_data_file_save(tmp_data, option_43);
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_66);
	option_data_file_save(tmp_data, option_66);
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_67);
	option_data_file_save(tmp_data, option_67);

	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_121);
	option_data_file_save(tmp_data, option_121);
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_125);
	option_data_file_save(tmp_data, option_125);
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_128);
	option_data_file_save(tmp_data, option_128);
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_132);
	option_data_file_save(tmp_data, option_132);
#ifdef XAVI_UDHCPC_OPT224
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_224);
	option_data_file_save(tmp_data, option_224);
#endif
#ifdef XAVI_UDHCPC_OPT250_APAS
	sprintf(tmp_data, "%s/%s/%s", _PATH_WAN_DIR, interface, _PATH_OPTION_250);
	option_data_file_save(tmp_data, option_250);

	int apasPid;
	if (strcmp(option_250, "")) {
		apasPid = prctl_getPidByName("apasd");
		sprintf(tmp_data, "kill -SIGUSR1 %d", apasPid);
		system(tmp_data);
	}
#endif
#endif
...
static char **fill_envp(struct dhcp_packet *packet)
{
	...
	*curr = xmalloc(sizeof("ip=255.255.255.255"));
	...
	sprint_nip(ipaddress, "", (uint8_t *) &packet->yiaddr);
#endif
	...
	while (*opt_name) {
		...
		*curr = xmalloc_optname_optval(temp, &dhcp_optflags[i], opt_name);
		...
		if (dhcp_optflags[i].code == DHCP_SUBNET) {
			...
			uint32_t subnet;
#if defined(XAVI_MODEL_NAME) || defined(XAVI_UDHCPC_OPTION)
			opt_len = sprint_nip(subnetmask, "", opt_val);
			subnetmask[opt_len] = '\0';
#endif
...
#if defined(XAVI_MODEL_NAME) || defined(XAVI_UDHCPC_OPTION)
		else if (dhcp_optflags[i].code == DHCP_ROUTER) {
			opt_len = sprint_nip(routerip, "", opt_val);
			routerip[opt_len] = '\0';
#if defined(XAVI_UDHCPC_OPTION)
		} else if (dhcp_optflags[i].code == DHCP_VENDOR_SPECIFIC) {
			char *opt43p = option_43;
			if (opt_val[0] == 1) {
				opt_len = opt_val[1];
				memcpy(opt43p, (char *)opt_val + 2, opt_len);
				opt43p += opt_len;
				strcat(opt43p, "\0");
				...
				memcpy(option_43, opt_val, opt_len);
				option_43[opt_len] = '\0';
			}
		} else if (dhcp_optflags[i].code == DHCP_TFTP_SERVER_NAME) {
			memcpy(option_66, opt_val, opt_len);
			option_66[opt_len] = '\0';
		} else if (dhcp_optflags[i].code == DHCP_BOOT_FILE) {
			memcpy(option_67, opt_val, opt_len);
			option_67[opt_len] = '\0';
		} else if (dhcp_optflags[i].code == DHCP_OPT125) {
			memcpy(option_125, opt_val, opt_len);
			option_125[opt_len] = '\0';
		} else if (dhcp_optflags[i].code == DHCP_HTTP_IP) {
			memcpy(option_128, opt_val, opt_len);
			option_128[opt_len] = '\0';
		} else if (dhcp_optflags[i].code == DHCP_8021Q_VLAN_ID) {
			memcpy(option_132, opt_val, opt_len);
			option_132[opt_len] = '\0';
#ifdef XAVI_UDHCPC_OPT224
		} else if (dhcp_optflags[i].code == DHCP_PRIVATE224) {
			memcpy(option_224, opt_val, opt_len);
			option_224[opt_len] = '\0';
#endif
#ifdef XAVI_UDHCPC_OPT250_APAS
		} else if (dhcp_optflags[i].code == DHCP_PRIVATE250) {
			memcpy(option_250, opt_val, opt_len);
			option_250[opt_len] = '\0';
#endif
#endif
		}
#endif
...
/* The rest has variable bit positions, need to be clever */
 ...
 leasefail:
		...
		case SIGUSR2:
			...
			timeout = INT_MAX;
#ifdef XAVI_UDHCPC_OPTION
			system("echo > /var/run/release.sent");
#endif
			continue;
		case SIGTERM:
