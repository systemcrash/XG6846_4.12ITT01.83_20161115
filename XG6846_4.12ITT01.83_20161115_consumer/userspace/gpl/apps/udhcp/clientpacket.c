...
unsigned long random_xid(void)
{
	...
	if (!initialized) {
/* XAVI Chris 20120910 Fixed time(0) always the same issue in first time */
#ifdef XAVI_UDHCPC_OPTION
		unsigned int seed = client_config.arp[2];

		seed = (seed << 8) + client_config.arp[3];
		seed = (seed << 8) + client_config.arp[4];
		seed = (seed << 8) + client_config.arp[5];
...
{
#ifdef XAVI_UDHCPC_OPTION
	char base_request_list[] = {DHCP_PARAM_REQ, 0, PARM_REQUESTS};
	char request_list[64] = {DHCP_PARAM_REQ, 0, PARM_REQUESTS};
	int optlen = sizeof(base_request_list) - 2;
	
	request_list[OPT_LEN] = optlen;
		
	if (ipv6rd_opt)
	{
		request_list[optlen + 2] = DHCP_6RD_OPT;
		optlen ++;
		request_list[OPT_LEN] = optlen;
	}
	...
	if (en_66)
	{
		request_list[optlen + 2] = DHCP_TFTP_SERVER;
		optlen ++;
		request_list[OPT_LEN] = optlen;
	}	
	if (en_67)
	{
		request_list[optlen + 2] = DHCP_BOOT_FILE;
		optlen ++;
		request_list[OPT_LEN] = optlen;
	}	
	if (en_43)
	{
		request_list[optlen + 2] = DHCP_VENDOR_SPECIFIC_INFO;
		optlen ++;
		request_list[OPT_LEN] = optlen;
	}	
	if (en_128)
	{
		request_list[optlen + 2] = DHCP_TFTP_SERVER_ADDR;
		optlen ++;
		request_list[OPT_LEN] = optlen;
	}	
	if (en_121)
	{
		request_list[optlen + 2] = DHCP_CLASSLESS_STATIC_ROUTE;
		optlen ++;
		request_list[OPT_LEN] = optlen;
	}	
	if (en_132)
	{
		request_list[optlen + 2] = DHCP_8021Q_VLAN;
		optlen ++;
		request_list[OPT_LEN] = optlen;
	}
