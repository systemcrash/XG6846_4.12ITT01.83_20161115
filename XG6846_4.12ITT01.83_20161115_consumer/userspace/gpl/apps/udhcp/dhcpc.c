...
static unsigned long relay_addr = 0;
#else
...
#ifdef XAVI_UDHCPC_OPTION		
		if (relay_addr != 0)
                 send_release(relay_addr, requested_ip); /* unicast */
             else	
#endif			 	
...
#endif
{
	...
	static struct option options[] = {
		...
		{"6rd",	no_argument,		0, '6'},
#ifdef XAVI_UDHCPC_OPTION
		{"op66",	no_argument,		0, 'F'},
		{"op67",	no_argument,		0, 't'},
		{"op43",	no_argument,		0, 'E'},
		{"op128",	no_argument,		0, 'T'},
		{"op121",	no_argument,		0, 'R'},
		{"op132",	no_argument,		0, 'V'},
#endif	
		{0, 0, 0, 0}
	};
...
// brcm
#ifdef XAVI_UDHCPC_OPTION
		c = getopt_long(argc, argv, "c:fH:hi:np:qr:s:d:v:6FtETRVI:D:O:S:P:", options, &option_index);
#else
		...
		if (retval == 0) {
			...
			case REQUESTING:
				if (packet_num < 3) {
					...
					{
                        if (relay_addr != 0)
							send_renew(xid, relay_addr, requested_ip); /* unicast */
						else	
#endif							
