...
CmsRet devCtl_getWanMacAddress(UINT8 *macAddrNum)
{
   ...
   ret = devCtl_boardIoctl(BOARD_IOCTL_GET_BASE_MAC_ADDRESS,
                            0,
                            (char *) baseMac,
                            MAC_ADDR_LEN,
                            0,
   ...
   memcpy((unsigned char *) &baseMacAddr, baseMac + 3, sizeof(baseMac) - 3);

   baseMacAddr >>= 8;

#if defined(XAVI_DG150) || defined(XAVI_VI580)
		/* WAN MAC = Base MAC + 3 */
		baseMacAddr = (baseMacAddr + 3) << 8;
#elif defined(XAVI_XG6846)
		/* WAN MAC = Base MAC */
		baseMacAddr = (baseMacAddr + 0) << 8;
#else 
		/* WAN MAC = Base MAC + 2 */
		baseMacAddr = (baseMacAddr + 2) << 8;
#endif

   memcpy( macAddrNum, baseMac, 3);
   memcpy( macAddrNum + 3, (unsigned char *) &baseMacAddr, sizeof(baseMac) - 3 );
   
   return ret;

}

CmsRet devCtl_setBaseMacAddress(UINT8 *macAddrNum)
{
   return(devCtl_boardIoctl(BOARD_IOCTL_SET_BASE_MAC_ADDRESS,
                            0,
                            (char *) macAddrNum,
                            MAC_ADDR_LEN,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_GET_SERIAL_NUMBER,
                            0,
                            (char *) serialNumber,
                            BUFLEN_32 - 1,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_SET_SERIAL_NUMBER,
                            0,
                            (char *) serialNumber,
                            BUFLEN_32 - 1,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_GET_DES_KEY,
                            0,
                            (char *) desKey,
                            BUFLEN_32 - 1,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_SET_DES_KEY,
                            0,
                            (char *) desKey,
                            BUFLEN_32 - 1,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_GET_WEP_KEY,
                            0,
                            (char *) wepKey,
                            BUFLEN_32 - 1,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_SET_WEP_KEY,
                            0,
                            (char *) wepKey,
                            BUFLEN_32 - 1,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_GET_MODEL_NAME,
                            0,
                            (char *) modelName,
                            BUFLEN_32 - 1,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_SET_MODEL_NAME,
                            0,
                            (char *) modelName,
                            BUFLEN_32 - 1,
                            0,
...
int devCtl_getRestoreDefaultConfig()
{
   int value;
   CmsRet ret;
   ret = devCtl_boardIoctl(BOARD_IOCTL_GET_RESTORE_DEFAULT_CONFIG,
                           0, NULL,
                           0, 0, &value);

   return (ret == CMSRET_SUCCESS ? value : 0);        
}

CmsRet devCtl_setRestoreDefaultConfig(int ucNotResetDefaultConfig)
{
   return(devCtl_boardIoctl(BOARD_IOCTL_SET_RESTORE_DEFAULT_CONFIG,
                            0,
                            NULL,
                            0,
                            (SINT32) ucNotResetDefaultConfig,
                            ""));
}
#endif

#ifdef XAVI_SFP_DDM_DEF
CmsRet devCtl_setSFP_A2_EEPROM(UINT8 maddr, UINT8 data)
{
    return(devCtl_boardIoctl(BOARD_IOCTL_SET_SFP_A2_EEPROM,
		               0,
		               NULL,
		               maddr,
		               data,
...
CmsRet devCtl_setSFP_A0_EEPROM(UINT8 maddr, UINT8 data)
{
    return(devCtl_boardIoctl(BOARD_IOCTL_SET_SFP_A0_EEPROM,
		               0,
		               NULL,
		               maddr,
		               data,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_GET_SFP_A0_EEPROM,
                            0,
                            NULL,
                            maddr,
                            0,
...
{
   return(devCtl_boardIoctl(BOARD_IOCTL_GET_SFP_A2_EEPROM,
                            0,
                            NULL,
                            maddr,
                            0,
...
{
   if (!strcmp(keyword, "ddm_vendor_name")) {
   	
#ifndef XAVI_CATV_MODULE
      UINT32 i, vendor_name[16];

      for (i = 0; i < 16; i++)
         devCtl_getSFP_A0_EEPROM((0x14 + i), &vendor_name[i]);
#else
      UINT32 i, vendor_name[20];

      for (i = 0; i < 20; i++)
         devCtl_getSFP_A0_EEPROM((12 + i), &vendor_name[i]);
#endif	  

      (vendor_name[0] == 0xFF) ? sprintf(data_string, "%s", "None"):

#ifndef XAVI_CATV_MODULE      
         sprintf(data_string, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
                 vendor_name[0],  vendor_name[1],  vendor_name[2],  vendor_name[3],
                 vendor_name[4],  vendor_name[5],  vendor_name[6],  vendor_name[7],
                 vendor_name[8],  vendor_name[9],  vendor_name[10], vendor_name[11],
                 vendor_name[12], vendor_name[13], vendor_name[14], vendor_name[15]);
#else
         sprintf(data_string, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
                 vendor_name[0],  vendor_name[1],  vendor_name[2],  vendor_name[3],
                 vendor_name[4],  vendor_name[5],  vendor_name[6],  vendor_name[7],
                 vendor_name[8],  vendor_name[9],  vendor_name[10], vendor_name[11],
                 vendor_name[12], vendor_name[13], vendor_name[14], vendor_name[15],
                 vendor_name[16], vendor_name[17], vendor_name[18], vendor_name[19]);
#endif

   } else if (!strcmp(keyword, "ddm_vendor_oui")) {

#ifndef XAVI_CATV_MODULE
      UINT32 i, vendor_oui[3];

      for (i = 0; i < 3; i++)
         devCtl_getSFP_A0_EEPROM((0x25 + i), &vendor_oui[i]);
#else	
      UINT32 i, vendor_oui[12];

      for (i = 0; i < 12; i++)
         devCtl_getSFP_A0_EEPROM((0 + i), &vendor_oui[i]);
#endif	  
	  
      (vendor_oui[0] == 0xFF) ? sprintf(data_string, "%s", "None"):
#ifndef XAVI_CATV_MODULE      
         sprintf(data_string, "%.2X %.2X %.2X",
                 vendor_oui[0], vendor_oui[1], vendor_oui[2]);
#else
         sprintf(data_string, "%c%c%c%c%c%c%c%c%c%c%c%c",
                 vendor_oui[0],  vendor_oui[1],  vendor_oui[2],  vendor_oui[3],
                 vendor_oui[4],  vendor_oui[5],  vendor_oui[6],  vendor_oui[7],
                 vendor_oui[8],  vendor_oui[9],  vendor_oui[10], vendor_oui[11]);
#endif
   } else if (!strcmp(keyword, "ddm_vendor_pn")) {  
#ifndef XAVI_CATV_MODULE
      UINT32 i, vendor_pn[16];

      for (i = 0; i < 16; i++)
         devCtl_getSFP_A0_EEPROM((0x28 + i), &vendor_pn[i]);
#else
      UINT32 i, vendor_pn[20];

      for (i = 0; i < 20; i++)
         devCtl_getSFP_A0_EEPROM((33 + i), &vendor_pn[i]);
#endif

      (vendor_pn[0] == 0xFF) ? sprintf(data_string, "%s", "None"):
#ifndef XAVI_CATV_MODULE      
         sprintf(data_string, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
                 vendor_pn[0],  vendor_pn[1],  vendor_pn[2],  vendor_pn[3],
                 vendor_pn[4],  vendor_pn[5],  vendor_pn[6],  vendor_pn[7],
                 vendor_pn[8],  vendor_pn[9],  vendor_pn[10], vendor_pn[11],
                 vendor_pn[12], vendor_pn[13], vendor_pn[14], vendor_pn[15]);
#else
         sprintf(data_string, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
                 vendor_pn[0],  vendor_pn[1],  vendor_pn[2],  vendor_pn[3],
                 vendor_pn[4],  vendor_pn[5],  vendor_pn[6],  vendor_pn[7],
                 vendor_pn[8],  vendor_pn[9],  vendor_pn[10], vendor_pn[11],
                 vendor_pn[12], vendor_pn[13], vendor_pn[14], vendor_pn[15],
                 vendor_pn[16], vendor_pn[17], vendor_pn[18], vendor_pn[19]);
#endif
   } else if (!strcmp(keyword, "ddm_vendor_rev")) {
      UINT32 i, vendor_rev[4];

#ifndef XAVI_CATV_MODULE
      for (i = 0; i < 4; i++)
         devCtl_getSFP_A0_EEPROM((0x38 + i), &vendor_rev[i]);
#else
      for (i = 0; i < 4; i++)
         devCtl_getSFP_A0_EEPROM((53 + i), &vendor_rev[i]);
#endif	  
      (vendor_rev[0] == 0xFF) ? sprintf(data_string, "%s", "None"):
         sprintf(data_string, "%c%c%c%c",
                 vendor_rev[0], vendor_rev[1], vendor_rev[2], vendor_rev[3]);
   } else if (!strcmp(keyword, "ddm_temperature")) {
      UINT32 msb_value = 0, lsb_value = 0;

#ifndef XAVI_CATV_MODULE
      devCtl_getSFP_A2_EEPROM((0x60 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((0x60 & 0xFE), &msb_value);
#else
      devCtl_getSFP_A2_EEPROM((58 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((58 & 0xFE), &msb_value);
#endif

      if (msb_value & 0x80) {
         lsb_value  = (256 - lsb_value);
         ((msb_value != 0xFF) && (lsb_value != 0xFF)) ? : (msb_value = 0x00);
         ((msb_value != 0xFF) && (lsb_value != 0x00)) ? : (msb_value = 0x01);
         ((msb_value == 0x80) ? : (msb_value &= 0x7F));
         sprintf(data_string, "-%.3f C", ((float)lsb_value / 256) + (float)msb_value);
      } else
         sprintf(data_string,  "%.3f C", ((float)lsb_value / 256) + (float)msb_value);
   } else if (!strcmp(keyword, "ddm_voltage")) {
      UINT32 msb_value = 0, lsb_value = 0;
	  
#ifndef XAVI_CATV_MODULE
      devCtl_getSFP_A2_EEPROM((0x62 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((0x62 & 0xFE), &msb_value);
#else
      devCtl_getSFP_A2_EEPROM((60 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((60 & 0xFE), &msb_value);
#endif

      (lsb_value == 0xFF) ? : ((msb_value != 0xFF) ? : ( msb_value = 0x00));
      ((msb_value != 0xFF) || (lsb_value != 0xFF)) ?

#ifndef XAVI_CATV_MODULE	  	
         sprintf(data_string, "%.2f Volts", ((float)((msb_value << 8) | lsb_value) / 10000)):
#else
         sprintf(data_string, "%.2f Volts", (2.44 * (1.0 + ((msb_value << 8) | lsb_value) / 512.0))):
#endif

         sprintf(data_string, "%s", "None");
   } else if (!strcmp(keyword, "ddm_bias")) {
      UINT32 msb_value = 0, lsb_value = 0;

      devCtl_getSFP_A2_EEPROM((0x64 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((0x64 & 0xFE), &msb_value);
      (lsb_value == 0xFF) ? : ((msb_value != 0xFF) ? : ( msb_value = 0x00));
      ((msb_value != 0xFF) || (lsb_value != 0xFF)) ?
         sprintf(data_string, "%d mA", (((msb_value << 8) | lsb_value) / 2000)):
         sprintf(data_string, "%s", "None");
   } else if (!strcmp(keyword, "ddm_tx_power")) {
      UINT32 msb_value = 0, lsb_value = 0;

      devCtl_getSFP_A2_EEPROM((0x66 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((0x66 & 0xFE), &msb_value);
      (lsb_value == 0xFF) ? : ((msb_value != 0xFF) ? : ( msb_value = 0x00));
      if ((msb_value != 0xFF) || (lsb_value != 0xFF)) {
         float tmp_value = 0;

         /* The value 7.35 = 48.2 / 6.5535 Range 0 to 6.5535 mW (~ -40 to +8.2 dBm). */
         //tmp_value = ((float)((msb_value << 8) | lsb_value) / 10000) * 7.35;	 
       //  (tmp_value < 40) ?
       //     sprintf(data_string, "-%.2f dBm", tmp_value):
       //     sprintf(data_string,  "%.2f dBm", (tmp_value - 40));

         tmp_value = (float)((msb_value << 8) | lsb_value) / 10000 ;
	     sprintf(data_string, "%.2f dBm", 10 * log10f(tmp_value));
      } else
         sprintf(data_string, "%s", "None");
   } else if (!strcmp(keyword, "ddm_rx_power")) {
      UINT32 msb_value = 0, lsb_value = 0;

      devCtl_getSFP_A2_EEPROM((0x68 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((0x68 & 0xFE), &msb_value);
      (lsb_value == 0xFF) ? : ((msb_value != 0xFF) ? : ( msb_value = 0x00));
      if ((msb_value != 0xFF) || (lsb_value != 0xFF)) {
         float tmp_value = 0;
   ...
   } else if (!strcmp(keyword, "ddm_rf_level")) {
	UINT32 msb_value = 0, lsb_value = 0;
	
      devCtl_getSFP_A2_EEPROM((64 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((64 & 0xFE), &msb_value);
      (lsb_value == 0xFF) ? : ((msb_value != 0xFF) ? : ( msb_value = 0x00));
      ((msb_value != 0xFF) || (lsb_value != 0xFF)) ?
         sprintf(data_string, "%.2f dBuV", (((msb_value << 8) | lsb_value )*(2.44/1024.0) + 0.9148 )/0.0582 + 60.0) :
         sprintf(data_string, "%s", "None");
		 
   } else if (!strcmp(keyword, "ddm_opt_level")) {
   	UINT32 msb_value = 0, lsb_value = 0, responsivity = 0;
	int resp;

      devCtl_getSFP_A2_EEPROM((62 | 0x01), &lsb_value);
      devCtl_getSFP_A2_EEPROM((62 & 0xFE), &msb_value);	

	devCtl_getSFP_A0_EEPROM(84, &responsivity);

	resp = responsivity;
	
      (lsb_value == 0xFF) ? : ((msb_value != 0xFF) ? : ( msb_value = 0x00));
      ((msb_value != 0xFF) || (lsb_value != 0xFF)) ?      		
         sprintf(data_string, "%.2f dBm", 10 * log10(((msb_value << 8) | lsb_value )*(2.44/1024.0)/(0.47*4.9)/(resp*0.01))) :
         sprintf(data_string, "%s", "None");	
	  
#endif
#ifdef XAVI_SFP_DDM_DEF
   } else if (!strcmp(keyword, "ddm_vendor_sn")) {
      UINT32 i, vendor_sn[16];

      for (i = 0; i < 16; i++)
         devCtl_getSFP_A0_EEPROM((0x44 + i), &vendor_sn[i]);

      (vendor_sn[0] == 0xFF) ?  sprintf(data_string, "%s", "None") :
         sprintf(data_string, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
                 vendor_sn[0],  vendor_sn[1],  vendor_sn[2],  vendor_sn[3],
                 vendor_sn[4],  vendor_sn[5],  vendor_sn[6],  vendor_sn[7],
                 vendor_sn[8],  vendor_sn[9],  vendor_sn[10], vendor_sn[11],
                 vendor_sn[12], vendor_sn[13], vendor_sn[14], vendor_sn[15]);
#endif
   } else
      sprintf(data_string, "The %s Command Not Found!", keyword);
...
CmsRet  devCtl_catvData (int enable)
{
  ...
  int bpGpio = BP_GPIO_7_AL;
  
  fprintf(stderr, "[%s][%d] catvData: enable %d\n", __FUNCTION__, __LINE__,
	enable);
  cmsLog_debug("catvData: enable %d", enable);
  return devCtl_boardIoctl(BOARD_IOCTL_SET_GPIO,
	  						0,
	  						NULL,
	  						bpGpio,
	  						(enable ? kGpioActive: kGpioInactive),
	  						NULL);
}
#endif
...
CmsRet  devCtl_catvData (int enable)
{
  ...
  int pin = BP_GPIO_39_AL;
  
  cmsLog_debug("devCtl_catvData: enable %d", enable);
...
CmsRet  devCtl_catvClk (int enable)
{
  ...
  int pin = BP_GPIO_30_AL;
  	  
  cmsLog_debug("devCtl_catvClk: enable %d", enable);
...
CmsRet  devCtl_catvSwitch (int module, int enable, int bwFilter)
{
  ...
  int i2c_data, i2c_clk;
  UINT32 value_in;
  UINT8 value_out;
  	
  i2c_data = BP_GPIO_39_AL;
  i2c_clk = BP_GPIO_30_AL;

  if (module == 0) {
  
  cmsLog_debug("devCtl_catvSwitch: enable %d", enable);
  ...
  cmsLog_debug("devCtl_catvSwitch: enable %d bwFilter %d", enable, bwFilter);  

#ifdef XAVI_SFP_DDM_DEF
  // set both pins to high
  devCtl_boardIoctl(BOARD_IOCTL_SET_GPIO,
	  						0,
	  						NULL, 
	  						i2c_data,
	  						kGpioInactive,
  ...
  // get A2 table address 73
  devCtl_getSFP_A2_EEPROM(73, &value_in);
 
 // write to address for filter and agc
  value_out = (value_in & 0x83) | enable << 2 | bwFilter << 3;
  return devCtl_setSFP_A2_EEPROM(73, value_out);
...
{
    int bpGpio = BP_GPIO_10_AH;

    cmsLog_debug("devCtl_FXSRelay: enable %d", enable);	
    return devCtl_boardIoctl(BOARD_IOCTL_SET_GPIO,
	  						0,
	  						NULL, 
	  						bpGpio,
...
// Get Wireless Switch state from GPIO
UINT32  devCtl_getWirelessSwitchState(void)
{
    UINT32 num;
    CmsRet ret;
	
    ret =  devCtl_boardIoctl(BOARD_IOCTL_GET_WIRELESS_SWITCH_STATE,
					0, NULL,
					0, 0, &num);	
...
CmsRet devCtl_getDefaultOpticalParams(char * pOpticalData)
{    
    int Ret;
    
    Ret = devCtl_boardIoctl(BOARD_IOCTL_GET_DEFAULT_OPTICAL_PARAMS, 0, pOpticalData, BP_OPTICAL_PARAMS_LEN, 0, "");
                         ...
                         void *data)
{
	...
	#ifdef XAVI_SFP_DDM_DEF
                  (boardIoctl == BOARD_IOCTL_GET_SFP_A0_EEPROM) ||
                  (boardIoctl == BOARD_IOCTL_GET_SFP_A2_EEPROM) ||
	#endif
    #if defined(XAVI_VI580) || defined(XAVI_FI560) || defined(XAVI_DG150)
                  (boardIoctl == BOARD_IOCTL_GET_WIRELESS_SWITCH_STATE) ||                   
    #endif
    #ifdef XAVI_RESTORE_DEFAULT_CONFIG_AFTER_TR69_SW_UPGRADE
                  (boardIoctl == BOARD_IOCTL_GET_RESTORE_DEFAULT_CONFIG) ||                   
    #endif
